...
///einladen aller Grafiken usw. f�r das Men�
bool cMenu::load()
{
...
void cMenu::update_main()
{
	...
	for(int i=0;i<4;i++)
	{
		...
		{
			text_main[i]=true;
			///wir schaun auch noch gerade nach wenn der mausbutton gedr�ckt wurde
			...
			if(pKeyboard->getmousepressed()>0)
			{
				...
				switch(i)
				{
					...
					case 2://suicide modus
						...
						//falls es schon array mit den ausgew�hlten Spielern gibt l�schen wir es hier
						if(playerselected)
...
///alle klassen sowie daten f�r das spiel werden erstellt
