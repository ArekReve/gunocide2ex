...
///erstellen der dummy tables f�r die highscore liste
...
///gibt true oder false zur�ck..je nachdem =)
bool cHScore::save()
{
	FILE	*file=NULL;
...
///anzeigen der Highscores abh�ngig vom gew�hlten Modus
void cHScore::render(SDL_Surface *screen)
{
	cFont	*pFont=cFont::getInstance();

	//eine for-schleife funzt hier nicht, da die abst�nde zwischen den einzelnen
	//score-pl�tzen der grafik nicht gleich gross sind..verflucht sei oli ;)
...
///�berpr�fen ob die score es in die highscore-liste geschafft hat
...
void cHScore::checkfornewentry(long tscore,int ratio,int waves,int gmode)
{
	...
	///wenn der score nicht mal den letzten platz schl�gt
	///k�nnen wir getrost rausgehen...der spieler ist einfach ein looser ;)
	...
	///f�r ne highscore-liste ben�tigen wir kein merge oder �hnliches
	while(!check)
	{
		check=true;
...
void cHScore::update()
{
	...
	//wir gehen einen buchstaben zur�ck
