...
bool cStage::load(char *path,cEnemyMan *pEnemyMan)
{
	...
	//Datei �ffnen und false zur�ckgeben wenn es schief geht
	file=fopen(path,"r");
	if(!file)
	{
	...
	///einladen der Gegnerdaten und dem EnemyManager hinzuf�gen
	while(!feof(file))
	{
		...
		fscanf(file,"%i",&balive);	//gibts den Gegner eigentlich? H�tte man damals sch�ner machen sollen ;)
